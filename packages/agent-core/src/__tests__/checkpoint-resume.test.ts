...
describe('checkpoint-resume with session entries', () => {
  ...
  it('should serialize session entries into checkpoint via onStepComplete pattern', () => {
    ...
    const state: AgentLoopState = {
      ...
      stepCount: 3,
      currentObservation: { summary: 'Login form visible', details: {}, timestamp: new Date().toISOString() },
      lastPlan: { reasoning: 'User needs to log in', action: 'Click username', toolName: 'browser_click', toolArgs: { ref: '42' }, expectedOutcome: 'Username field focused' },
  ...
  it('should restore session entries on resume', () => {
    ...
    const checkpoint: AgentCheckpoint = {
      ...
      config: { maxSteps: 20 },
      lastObservation: { summary: 'App running', details: {}, timestamp: '2026-01-01T00:00:01.000Z' },
      lastPlan: { reasoning: 'Need to click', action: 'Click button', toolName: 'browser_click', toolArgs: {}, expectedOutcome: 'Button clicked' },
  ...
  it('should preserve entries across multiple checkpoint saves', () => {
    ...
    const cp2: AgentCheckpoint = {
      ...
      taskPrompt: 'Long running task',
      config: { maxSteps: 20 },
      lastObservation: { summary: 'Step 2 observation', details: {}, timestamp: '2026-01-01T00:00:02.000Z' },
      lastPlan: { reasoning: 'step 2 plan', action: 'do step 2', toolName: 'browser_click', toolArgs: {}, expectedOutcome: 'step 2 done' },
      lastExecutionResult: { success: true, result: null },
      sessionEntries: sm.getSession(sessionId)?.entries ?? [],
      timestamp: new Date().toISOString(),
    };
